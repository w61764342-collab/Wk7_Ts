...
class KCSBScraper:
    def __init__(self, access_key, secret_key, bucket_name, endpoint_url, max_workers=5):
        ...
        # -----------------------------------------------------------------
        # Cloudflare R2 client — differences from AWS S3:
        #   1. endpoint_url is REQUIRED (custom R2 endpoint per account)
        #   2. signature_version must be explicitly set to 's3v4'
        #   3. addressing_style must be 'path' (R2 does not support virtual-hosted style)
        #   4. region_name is a dummy value; R2 ignores it
        # -----------------------------------------------------------------
                                ...
                                if '__doPostBack' in href:
                                    ...
                                    if match:
                                        ...
                                        if ('application/pdf' in content_type or
                                            ...
                                            if len(content) > 1000 or content[:4] == b'%PDF' or content[:2] == b'PK':
                                                # Generate R2 path for this child file in a subfolder
    ...
    def file_exists_in_s3(self, s3_path):
        """Check if file already exists in R2 with caching"""
        # Check cache first
    ...
    def batch_check_s3_exists(self, s3_paths):
        """Batch check if multiple files exist in R2"""
        results = {}
    ...
    def upload_to_s3(self, file_content, s3_path):
        """Upload file to Cloudflare R2 with thread safety"""
        try:
            with self.s3_lock:
            ...
            # Update cache
            self.s3_exists_cache[s3_path] = True
            logger.info(f"Uploaded to R2: {s3_path}")
            return True
        except NoCredentialsError:
            logger.error("Cloudflare R2 credentials not found")
            return False
        except Exception as e:
    ...
    def scrape_category(self, category_info):
        ...
        for tab in tabs:
            ...
            for idx, file_info in enumerate(files, 1):
                ...
                # Create R2 path  (same partition structure as S3)
    ...
    def run(self, filter_main_category=None):
        ...
        if filter_main_category:
            ...
            logger.info("Starting KCSB data scraping (R2) for ALL categories...")
        ...
        # Final summary
        logger.info("\n" + "="*50)
        logger.info("SCRAPING COMPLETE (Cloudflare R2)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Scrape KCSB data and upload to Cloudflare R2')
    parser.add_argument(
        '--category',
        type=str,
    ...
    # -----------------------------------------------------------------
    # Cloudflare R2 credentials — loaded from environment variables.
    # R2 always requires explicit credentials; no default chain exists.
    #
    ...
    #   CF_R2_BUCKET_NAME        — target bucket name
    # -----------------------------------------------------------------
    ...
    if not all([access_key, secret_key, endpoint_url, bucket_name]):
        logger.error(
            ...
            "Set CF_R2_ACCESS_KEY_ID, CF_R2_SECRET_ACCESS_KEY, "
            "CF_R2_ENDPOINT_URL, and CF_R2_BUCKET_NAME environment variables."
        )
        exit(1)
    ...
    logger.info(f"Starting R2 scraper with {max_workers} worker(s)")
    scraper = KCSBScraper(access_key, secret_key, bucket_name, endpoint_url, max_workers=max_workers)
